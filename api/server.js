/**
 * API Server - CephasGM AI Ecosystem
 * Complete REST API for AI ecosystem
 */
const express = require("express");
const cors = require("cors");
const helmet = require("helmet");
const compression = require("compression");

const os = require("../ecosystem/ai-os");
const factory = require("../ecosystem/agent-factory");
const nodeManager = require("../cluster/node-manager");
const vectorDb = require("../memory/vector-db");
const knowledgeGraph = require("../memory/knowledge-graph");
const textEngine = require("../multimodal/text-engine");
const imageEngine = require("../multimodal/image-engine");
const audioEngine = require("../multimodal/audio-engine");
const videoEngine = require("../multimodal/video-engine");

const app = express();
const PORT = process.env.PORT || 6000;

// Middleware
app.use(helmet());
app.use(cors());
app.use(compression());
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ extended: true }));

// Request logging
app.use((req, res, next) => {
  console.log(`\n📡 [${new Date().toISOString()}] ${req.method} ${req.path}`);
  next();
});

// ============================================
// System Endpoints
// ============================================

app.get("/health", (req, res) => {
  res.json({
    status: "healthy",
    service: "CephasGM AI Ecosystem",
    version: "6.0.0",
    timestamp: new Date().toISOString(),
    uptime: process.uptime()
  });
});

app.get("/status", (req, res) => {
  res.json({
    os: os.getStatus(),
    factory: factory.getStats(),
    cluster: nodeManager.getStatus(),
    memory: {
      vectors: vectorDb.getStats(),
      knowledge: knowledgeGraph.getStats()
    }
  });
});

// ============================================
// Agent Factory Endpoints
// ============================================

app.post("/create-agent", (req, res) => {
  try {
    const { type, config } = req.body;
    
    if (!type) {
      return res.status(400).json({ error: "Agent type is required" });
    }

    const agent = os.createAgent(type, config);
    
    res.json({
      success: true,
      agent
    });

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.get("/agents", (req, res) => {
  try {
    const agents = os.listAgents();
    res.json({
      success: true,
      count: agents.length,
      agents
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.delete("/agents/:agentId", (req, res) => {
  try {
    const { agentId } = req.params;
    const result = os.destroyAgent(agentId);
    
    res.json({
      success: result,
      agentId
    });

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// Task Execution Endpoints
// ============================================

app.post("/task", async (req, res) => {
  try {
    const { task, options } = req.body;
    
    if (!task) {
      return res.status(400).json({ error: "Task is required" });
    }

    const result = await os.runTask(task, options);
    
    res.json({
      success: true,
      ...result
    });

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/tasks", async (req, res) => {
  try {
    const { tasks, options } = req.body;
    
    if (!Array.isArray(tasks)) {
      return res.status(400).json({ error: "Tasks must be an array" });
    }

    const results = await os.runTasks(tasks, options);
    
    res.json({
      success: true,
      count: results.length,
      results
    });

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// Multimodal Endpoints
// ============================================

app.post("/generate/text", async (req, res) => {
  try {
    const { prompt, options } = req.body;
    
    if (!prompt) {
      return res.status(400).json({ error: "Prompt is required" });
    }

    const result = await textEngine.generate(prompt, options);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/generate/image", async (req, res) => {
  try {
    const { prompt, options } = req.body;
    
    if (!prompt) {
      return res.status(400).json({ error: "Prompt is required" });
    }

    const result = await imageEngine.generate(prompt, options);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/generate/audio", async (req, res) => {
  try {
    const { text, options } = req.body;
    
    if (!text) {
      return res.status(400).json({ error: "Text is required" });
    }

    const result = await audioEngine.generate(text, options);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/generate/video", async (req, res) => {
  try {
    const { prompt, options } = req.body;
    
    if (!prompt) {
      return res.status(400).json({ error: "Prompt is required" });
    }

    const result = await videoEngine.generate(prompt, options);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// Cluster Endpoints
// ============================================

app.get("/cluster/nodes", (req, res) => {
  try {
    const status = nodeManager.getStatus();
    res.json(status);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/cluster/execute", async (req, res) => {
  try {
    const { type, task, options } = req.body;
    
    if (!type || !task) {
      return res.status(400).json({ error: "Type and task are required" });
    }

    const result = await nodeManager.execute(type, task, options);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/cluster/strategy", (req, res) => {
  try {
    const { strategy } = req.body;
    
    if (!strategy) {
      return res.status(400).json({ error: "Strategy is required" });
    }

    const result = nodeManager.setStrategy(strategy);
    res.json({ success: true, strategy: result });

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// Memory Endpoints
// ============================================

app.post("/memory/vector", (req, res) => {
  try {
    const { vector, metadata } = req.body;
    
    if (!vector) {
      return res.status(400).json({ error: "Vector is required" });
    }

    const result = vectorDb.add(vector, metadata);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/memory/vector/search", (req, res) => {
  try {
    const { query, limit, threshold } = req.body;
    
    if (!query) {
      return res.status(400).json({ error: "Query is required" });
    }

    const result = vectorDb.search(query, limit, threshold);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.post("/memory/knowledge/link", (req, res) => {
  try {
    const { source, relation, target, properties } = req.body;
    
    if (!source || !relation || !target) {
      return res.status(400).json({ error: "Source, relation, and target are required" });
    }

    const result = knowledgeGraph.link(source, relation, target, properties);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.get("/memory/knowledge/:entity", (req, res) => {
  try {
    const { entity } = req.params;
    const { depth } = req.query;
    
    const result = knowledgeGraph.query(entity, depth ? parseInt(depth) : 1);
    res.json(result);

  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// Model Endpoints
// ============================================

app.get("/models", (req, res) => {
  try {
    const models = {
      text: textEngine.getModels(),
      image: imageEngine.getModels(),
      cluster: nodeManager.getNodeCountByType()
    };
    
    res.json(models);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// Root Endpoint
// ============================================

app.get("/", (req, res) => {
  res.json({
    name: "CephasGM AI Ecosystem",
    version: "6.0.0",
    description: "Complete AI Infrastructure Platform",
    endpoints: {
      system: {
        "GET /health": "Health check",
        "GET /status": "System status"
      },
      agents: {
        "POST /create-agent": "Create new agent",
        "GET /agents": "List all agents",
        "DELETE /agents/:id": "Delete agent"
      },
      tasks: {
        "POST /task": "Execute single task",
        "POST /tasks": "Execute multiple tasks"
      },
      multimodal: {
        "POST /generate/text": "Generate text",
        "POST /generate/image": "Generate image",
        "POST /generate/audio": "Generate audio",
        "POST /generate/video": "Generate video"
      },
      cluster: {
        "GET /cluster/nodes": "List cluster nodes",
        "POST /cluster/execute": "Execute on cluster"
      },
      memory: {
        "POST /memory/vector": "Add vector",
        "POST /memory/vector/search": "Search vectors",
        "POST /memory/knowledge/link": "Create knowledge link",
        "GET /memory/knowledge/:entity": "Query knowledge graph"
      }
    },
    documentation: "https://docs.cephasgm.ai"
  });
});

// Error handling
app.use((err, req, res, next) => {
  console.error('Unhandled error:', err);
  res.status(500).json({ error: 'Internal server error' });
});

// 404 handler
app.use((req, res) => {
  res.status(404).json({ error: 'Endpoint not found' });
});

// Start server
app.listen(PORT, () => {
  console.log(`
╔════════════════════════════════════════════════════════════════╗
║                                                                ║
║   🚀 CephasGM AI Ecosystem v6.0                               ║
║   📍 API Server running on port ${PORT}                        ║
║   🌐 Complete AI Infrastructure Platform                       ║
║                                                                ║
║   Features:                                                    ║
║   • Self-building agents                                       ║
║   • Autonomous research labs                                   ║
║   • AI software factory                                        ║
║   • Multimodal AI engine                                       ║
║   • Distributed worker nodes                                   ║
║   • Vector + knowledge memory                                  ║
║   • Scalable AI operating system                               ║
║                                                                ║
║   📝 API Documentation: http://localhost:${PORT}                ║
║                                                                ║
╚════════════════════════════════════════════════════════════════╝
  `);
});

module.exports = app;
